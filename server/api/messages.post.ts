...
try {
    ...
    if (eventError) console.error('[Messages POST] Event Error:', eventError)
    console.log('[Messages POST] Active Event ID:', activeEvent?.id)
    ...
    if (roomError) {
        console.error('[Messages POST] Room Error:', roomError)
        // Force create if missing logic is here...
    }
    console.log('[Messages POST] Target Room ID:', room?.id)

    if (!room) throw new Error('General room could not be resolved or created.')
